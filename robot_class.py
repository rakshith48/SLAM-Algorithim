...
class robot:
    ...
    def sense(self):
        ...
        for i, landmark in enumerate(self.landmarks):
            dlx = landmark[0] - self.x + self.rand() * self.measurement_noise
            dly = landmark[1] - self.y + self.rand() * self.measurement_noise
            if abs(dlx) <= self.measurement_range and abs(dly) <= self.measurement_range:
                measurements.append([i, dlx, dly])
        return measurements
